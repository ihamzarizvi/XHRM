...
use XHRM\Admin\Service\EmailConfigurationService;
use XHRM\Config\Config;
use XHRM\Core\Dao\EmailDao;
use XHRM\Core\Exception\CoreServiceException;
use XHRM\Core\Exception\ServiceException as Exception;
use XHRM\Core\Mail\AbstractRecipient;
use XHRM\Core\Mail\MailProcessor;
use XHRM\Core\Mail\TemplateHelper;
...
use XHRM\Core\Utility\Mailer;
use XHRM\Core\Utility\MailMessage;
use XHRM\Core\Utility\MailTransport;
use XHRM\Entity\EmailConfiguration;
use XHRM\Entity\EmailTemplate;
...
class EmailService
{
    ...
    protected function loadConfiguration(): void
    {
        ...
        if (
            $emailConfig instanceof EmailConfiguration && in_array(
    ...
    public function getTransport(): ?MailTransport
    {
        ...
        if ($this->isConfigSet()) {
            if (
                ...
                if (
                    $this->getEmailConfig()->getSmtpSecurityType() == self::SMTP_SECURITY_SSL ||
    ...
    public function getMessage()
    {
        ...
        if (empty($this->messageFrom)) {
            ...
            $this->messageFrom = [$this->getEmailConfig()->getSentAs() => "XHRM"];
        }
    ...
    public function sendTestEmail(string $toEmail): bool
    {
        ...
        $body = "This email confirms that $type details set in XHRM ";
