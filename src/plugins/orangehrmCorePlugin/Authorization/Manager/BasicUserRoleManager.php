...
use XHRM\Core\Authorization\Dao\HomePageDao;
...
use XHRM\Core\Authorization\Service\DataGroupService;
use XHRM\Core\Authorization\Service\ScreenPermissionService;
use XHRM\Core\Authorization\UserRole\AbstractUserRole;
use XHRM\Core\Exception\CoreServiceException;
use XHRM\Core\HomePage\HomePageEnablerInterface;
...
class BasicUserRoleManager extends AbstractUserRoleManager
{
    ...
    private function _init(): void
    {
        ...
        foreach ($configurations as $roleName => $roleObj) {
            $className = $this->getClassHelper()->getClass(
                $roleObj['class'],
                'XHRM\\Core\\Authorization\\UserRole\\'
            );
    ...
    public function getModuleDefaultPage(string $module): ?string
    {
        ...
        foreach ($defaultPages as $defaultPage) {
            ...
            $fallbackNamespace = 'XHRM\\Core\\HomePage\\';
