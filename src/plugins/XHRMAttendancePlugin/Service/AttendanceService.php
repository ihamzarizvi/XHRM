...
namespace XHRM\Attendance\Service;

use XHRM\Attendance\Dao\AttendanceDao;
use XHRM\Core\Service\AccessFlowStateMachineService;
...
class AttendanceService
{
     ...
     * @throws \Exception
     */
    public function importAttendanceData($file)
    {
        ...
        if (($handle = fopen($file->getPathname(), "r")) !== FALSE) {
            ...
            // Re-map format detection
            // Header: "Name","No.","Date/Time","Location ID","ID Number"
            $isRawFormat = false;
            $idxNo = false;
            $idxDateTime = false;
            ...
            if ($isRawFormat) {
                // --- RAW FORMAT LOGIC ---
                $rawPunches = []; // [employee_id => [date => [timestamps]]]

                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    $employeeId = $data[$idxNo] ?? null;
                    $dateTimeStr = $data[$idxDateTime] ?? null;

                    if (empty($employeeId) || empty($dateTimeStr)) {
                        continue;
                    }

                    // Parse Date/Time: 26-12-2025 10:41
                    try {
                        $dt = \DateTime::createFromFormat('d-m-Y H:i', $dateTimeStr, new \DateTimeZone('UTC'));
                        if ($dt) {
                            $dateKey = $dt->format('Y-m-d');
                            $rawPunches[$employeeId][$dateKey][] = $dt;
                        }
                    } catch (\Exception $e) {
                ...
                // Process Aggregated Punches
                foreach ($rawPunches as $empId => $dates) {
                    $employee = $this->getAttendanceDao()->getEmployeeByEmpNumber((int) $empId);
                    if (!$employee) {
                        $results['errors'][] = "Employee not found for ID '{$empId}'.";
                        continue;
                    }

                    foreach ($dates as $date => $punches) {
                        // Sort punches: Earliest is In, Latest is Out
                        usort($punches, function ($a, $b) {
                            return $a <=> $b; });

                        $count = count($punches);
                        $punchIn = $punches[0];
                        $punchOut = ($count > 1) ? $punches[$count - 1] : null;

                        try {
                            $attendanceRecord = new AttendanceRecord();
                            $attendanceRecord->setEmployee($employee);
                            $attendanceRecord->setPunchInUserTime($punchIn);
                            $attendanceRecord->setPunchInUtcTime($punchIn);
                            $attendanceRecord->setPunchInTimeOffset(0);
                            $attendanceRecord->setState(AttendanceRecord::STATE_PUNCHED_IN);

                            $this->getAttendanceDao()->savePunchRecord($attendanceRecord);

                            if ($punchOut && $punchOut > $punchIn) {
                                $attendanceRecord->setPunchOutUserTime($punchOut);
                                $attendanceRecord->setPunchOutUtcTime($punchOut);
                                $attendanceRecord->setPunchOutTimeOffset(0);
                                $attendanceRecord->setState(AttendanceRecord::STATE_PUNCHED_OUT);
                                $this->getAttendanceDao()->savePunchRecord($attendanceRecord);
                            }
                            $results['successCount']++;

                        } catch (\Exception $e) {
                            $results['errors'][] = "Error saving record for Emp {$empId} on {$date}: " . $e->getMessage();
                        }
                    }
                }
                ...
                // --- STANDARD LOGIC ---
                // We consumed head, but standard logic might not have headers or different headers.
                // Assuming standard CSV has headers like EmployeeID, PunchIn... 
                // If standard logic assumes headers, we are good.
                // If standard logic assumes NO headers, we just consumed the first row.
                // Per previous requirements: "Expected Format: EmployeeID, PunchInTime...". 
                // The previous code had `array_shift` or `fgetcsv` to skip header.
                // So skipping header is correct.

                $rowNumber = 1;
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    $rowNumber++;
                    // Expected Format: EmployeeID, PunchInTime (Y-m-d H:i:s), PunchOutTime (Y-m-d H:i:s), TimezoneOffset
                    $employeeId = $data[0] ?? null;
                    $punchInTime = $data[1] ?? null;
                    $punchOutTime = $data[2] ?? null;
                    $timezoneOffset = $data[3] ?? 0;

                    if (empty($employeeId) || empty($punchInTime)) {
                        $results['errors'][] = "Row {$rowNumber}: Missing Employee ID or Punch In Time.";
                        continue;
                    }

                    $employee = $this->getAttendanceDao()->getEmployeeByEmpNumber((int) $employeeId);
                    if (!$employee) {
                        $results['errors'][] = "Row {$rowNumber}: Employee not found for ID '{$employeeId}'.";
                        continue;
                    }

                    try {
                        $punchInDateTime = new \DateTime($punchInTime);
                        $punchInDateTime->setTimezone(new \DateTimeZone('UTC'));

                        $attendanceRecord = new AttendanceRecord();
                        $attendanceRecord->setEmployee($employee);
                        $attendanceRecord->setPunchInUserTime($punchInDateTime);
                        $attendanceRecord->setPunchInUtcTime($punchInDateTime);
                        ...
                        if (!empty($punchOutTime)) {
                            $punchOutDateTime = new \DateTime($punchOutTime);
                            $attendanceRecord->setPunchOutUserTime($punchOutDateTime);
                            $attendanceRecord->setPunchOutUtcTime($punchOutDateTime);
                        ...
                        $results['errors'][] = "Row {$rowNumber}: Error saving record - " . $e->getMessage();
                    }
                }
            }
            fclose($handle);
