<template>
  <oxd-dialog :is-open="isOpen" @close="$emit('close')">
    ...
    <div class="oxd-form-row">
       <oxd-input-field v-model="email" label="User Email" placeholder="Enter email to share with" />
    </div>
    ...
    <template #footer>
       <oxd-button @click="$emit('close')" ghost label="Cancel" />
       <oxd-button @click="share" label="Share" />
    </template>
  </oxd-dialog>
</template>
